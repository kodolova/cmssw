		...
		if ((hpos[i][depth][sub] && (hpos[i][depth][sub])->Integral()!=0) && (hneg[i][depth][sub] && (hneg[i][depth][sub])->Integral()!=0))
		  {
		    ...
		    posxaxis->SetRange(35,75);
		    // posyaxis->SetRangeUser(0,45);
		    negxaxis->SetRange(35,75);
		    //negyaxis->SetRangeUser(0,45);
			...
			std::cout << "Chisquare: " << pos_chisq << "/" << pos_ndf << " : " << pos_chisqdf << endl;                                                           double pos_mean = pos_gfit->GetParameter(1); 
			double pos_sigma = pos_gfit->GetParameter(2);       
			std::cout << "title" << title << " " << "meanvalue" << " " << pos_mean << " " << "sigma" << pos_sigma << std::endl;
			...
			std::cout << "Chisquare: " << neg_chisq << "/" << neg_ndf << " : " << neg_chisqdf << endl;                               			     double neg_mean = neg_gfit->GetParameter(1);
			double neg_sigma = neg_gfit->GetParameter(2);
			//gStyle->SetOptStat(111111);
			std::cout << "title" << title1 << " " << "meanvalue" << " " << neg_mean << " " << "sigma" << neg_sigma << std::endl;
