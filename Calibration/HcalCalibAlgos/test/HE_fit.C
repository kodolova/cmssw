...
void fit(char* type1 = "plus", char* type2 = "minus")
{
  ...
  char mycharout[100];
  sprintf(mycharout,"MeanVar_HE.txt");
  ...
  if (!File) {
	...
	for (int i=0;i<41;i++)
	  {
	    sprintf (hnamepos,"meanVar_%s for HE for eta %d depth %d subd 2",type1,(i+1),(depth+1));
	    sprintf (hnameneg,"meanVar_%s for HE for eta %d depth %d subd 2",type2,(i+1),(depth+1));
	    ...
	    if ((hpos[i][depth] && (hpos[i][depth])->Integral()!=0) && (hneg[i][depth] && (hneg[i][depth])->Integral()!=0))
	      {
		if((hpos[i][depth]->GetEntries())==72 && (hneg[i][depth]->GetEntries())==72)
		  {
			...
			sprintf (cname, "HE_%sCanvas_for_eta%d depth%d subd2" ,type1, (i+1), (depth+1));
			sprintf (title, "meanVar_%s for HE for eta%d depth%d subd2",type1, (i+1), (depth+1));
			sprintf (cname1, "HE_%sCanvas_for_eta%d depth%d subd2" ,type2, (i+1), (depth+1));
			sprintf (title1, "meanVar_%s for HE for eta%d depth%d subd2",type2, (i+1), (depth+1));
			...
			if (type1 && type2)
			  {
			    fprintf(Out,"%d %d 2 %f %f\n",(i+1),(depth+1),pos_mean,pos_sigma);
			    fprintf(Out,"%d %d 2 %f %f\n",(-1)*(i+1),(depth+1),neg_mean,neg_sigma);
			  }
		      }
		  }
		else if((hpos[i][depth]->GetEntries())==36 && (hneg[i][depth]->GetEntries())==36)
		  {
		    double pos_mean=hpos[i][depth]->GetMean();
		    double neg_mean=hneg[i][depth]->GetMean();
		    double pos_err=hpos[i][depth]->GetRMS();
		    double neg_err=hneg[i][depth]->GetRMS();
		    fprintf(Out,"%d %d 2 %f %f\n",(i+1),(depth+1),pos_mean,pos_err);
		    fprintf(Out,"%d %d 2 %f %f\n",(-1)*(i+1),(depth+1),neg_mean,neg_err);
		  }
	      }
	  }
      }
